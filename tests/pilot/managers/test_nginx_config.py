...
# --- reloading waits for the config to be live -------------------------------


def test_a_reload_returns_once_the_new_workers_are_up(monkeypatch) -> None:
    """Reloading only signals nginx; the workers serving the new config appear a
    moment later, and until then the running ones answer."""
    from pilot.managers import nginx as nginx_module

    samples = iter([{"1", "2"}, {"1", "2"}, {"1", "2", "3"}])
    monkeypatch.setattr(nginx_module, "nginx_worker_pids", lambda: next(samples))

    nginx_module.await_reloaded_workers({"1", "2"}, timeout=2)

    # The remaining sample is untouched: it stopped at the first new worker.
    assert next(samples, "exhausted") == "exhausted"


def test_no_wait_when_the_workers_cannot_be_listed(monkeypatch) -> None:
    from pilot.managers import nginx as nginx_module

    looked = []
    monkeypatch.setattr(nginx_module, "nginx_worker_pids", lambda: looked.append(1) or set())

    nginx_module.await_reloaded_workers(set(), timeout=2)

    assert looked == []


def test_a_reload_that_never_rotates_gives_up(monkeypatch) -> None:
    """Bounded: an nginx that does not fork must not hang the caller."""
    import time

    from pilot.managers import nginx as nginx_module

    monkeypatch.setattr(nginx_module, "nginx_worker_pids", lambda: {"1", "2"})
    started = time.monotonic()

    nginx_module.await_reloaded_workers({"1", "2"}, timeout=0.2)
