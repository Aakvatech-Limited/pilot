...
A rename never drops traffic already on the site. Requests keep arriving on the old hostname and keep being answered; the new hostname starts serving a moment later, once nginx has finished reloading - a reload is asynchronous, so the workers still running answer until the new ones are up. Two things would otherwise break in that gap, and both are handled:
...
A certbot lineage lives in the host-wide `/etc/letsencrypt` and is named after a hostname, so a pinned one claims that hostname exactly as a domain does: no site on any bench, and not the admin, can take it while it is pinned. Releasing a hostname whose certificate still serves the site's other TLS domains therefore completes in two steps - the rename keeps the pin so those domains stay on HTTPS, and the next `pilot setup letsencrypt` issues a certificate under the new name, drops the pin, and frees the old one. `cert_name` is protected from the site configuration API; only a rename writes it.

HTTPS survives the move as well. A certbot lineage is named after the site it was first issued for, so a rename would otherwise orphan the certificate: nginx would look under the new name, find nothing, and drop every one of the site's TLS domains to HTTP until a fresh one was issued. The rename pins the site's `cert_name` to the existing lineage instead, so the certificate keeps serving the hostnames it already covers, and the reissue expands that same lineage to the new name.
